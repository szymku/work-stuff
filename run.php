...
use Nrh\SapClient\SoapException;
...
//constants for sap-webservice-client
define('WEBSERVICE_TIMEOUT', 120);
define('WEBSERVICE_NWAIT', 600);
define('WEBSERVICE_STATUS_OK', 0);
//webservice name
define('Z_CSSRC_GOVE_UPL', 'Z_CSSRC_GOVE_UPL');

// load common libs
require_once dirname(__FILE__) . '/../lib/common-cli.inc.php';

// create sap webservice
$sapWebservice = Core::get('sap_webservice');
$webserviceParams = [];
...
try {
	...
	$sql = "
			...
			WHERE src.source_id in (32,3511,182,3651,3611,3271)
		";
...
//		1231,1411,1451,1491,1492,1531,1593,10363
	...
	foreach ($sources as $row) {
		try {
			...
			$feed = new Supplier_Feed($db, $row['source_id'], $sapWebservice, $output_log);
		...
		try {
			...
			foreach ($articles as $article_id => $fields) {
				$IT_IDATS = array(
					...
					'IDNLF' => mb_strtoupper($fields['external_id'], CHAR_SET),
				...
				if (!in_array($pure_sup_id, $supplier_without_call_sap)) {
					...
					$webserviceParams['IT_IDATS'][] = $IT_IDATS;
				}
			}
			...
			if (isset($final_unknown_ids[$sup_id])) {
				foreach ($final_unknown_ids[$sup_id] as $article_id => $fields) {
					$binds = array();
					$binds['log_id'] = $log_id;
					$binds['unknown_id'] = mb_strtoupper($article_id, CHAR_SET);
					$binds['status'] = null;
					...
					if (isset($fields['stock']) && $fields['stock'] === 0) {
						$IT_IDATS = array(
							'LIFNR' => $fields['supplier_id'],
							'IDNLF' => isset($fields['external_id']) ? mb_strtoupper($fields['external_id'], CHAR_SET) : "",
			...
			if (!in_array($pure_sup_id, $supplier_without_call_sap)) {
				...
				do {
					...
					$webserviceParams['IV_NWAIT'] = WEBSERVICE_NWAIT;
					$webserviceParams['ET_BAPIRET2'] = [];


					var_dump('IT_IDATS   ----COUNT----', count($webserviceParams['IT_IDATS']));


					$webserviceResponse = $sapWebservice->call(Z_CSSRC_GOVE_UPL, $webserviceParams, WEBSERVICE_TIMEOUT);

					if (isset($webserviceResponse[0])) {
						$webserviceResponse = $webserviceResponse[0];
					} else {
						throw new SoapException("Expected response from sap webservice not found");
					}
					...
					$error_cnt = $webserviceResponse->EV_CNT_ERR;

					// set ok count
					$ok_cnt = $webserviceResponse->EV_CNT;

					// get error messages
					$sap_err = $webserviceResponse->ET_BAPIRET2;

					$errors = [];

					if ($sap_err->item !== null) {
						foreach ($sap_err->item as $error) {

							if ($error->ID == 'ZUPL_MSG' && $error->NUMBER == 001) {
								$locked = true;
								$locked_msg = $error->MESSAGE;
							} else {
