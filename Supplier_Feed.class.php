<?php
use Nrh\SapClient\WebServiceClient;
...
class Supplier_Feed
{
	 ...
	 * @param WebServiceClient $sapWebservice
	 */
	public function __construct($db, $source_id, $sapWebservice, $logger)
	{
		$this->db = $db;
		$this->logger = $logger;
	...
	protected function get_known_ids()
	{
		...
		if (!$this->sapWebservice) {
			throw new Exception('There is no such a service in configuration');
		}

		$wsParams['IV_LIFNR'] = str_pad($this->feed_attributes['supplier_id'], 10, '0', STR_PAD_LEFT);
		$wsParams['IV_EKORG'] = $this->feed_attributes['purchase_organization'];

		switch ($this->feed_attributes['identify_by']) {
			case self::IDENTIFY_BY_EXTERNAL_ID:
				$wsParams['IV_IDENTBY'] = 'IDNLF';
				$col = 'IDNLF';
				break;
			case self::IDENTIFY_BY_EAN:
				$wsParams['IV_IDENTBY'] = 'EAN';
				$col = 'EAN11';
				break;
		...
		$wsResponse = $this->sapWebservice->call($wsName, $wsParams, WS_TIMEOUT);
		...
		foreach ($wsResponse[0] as $wsResponseKey => $wsValue) {

			if ('ET_V_EINE' === $wsResponseKey && $wsValue->item !== null) {

				foreach ($wsValue->item as $index => $row) {
					$id = mb_strtolower($row->$col, 'UTF-8');
					$this->known_ids[$id] = $id;
					$this->unknown_ids[$id] = array();

					if ($this->feed_attributes['identify_by'] == self::IDENTIFY_BY_EAN) {
						$this->matnr_eans[$row->MATNR][] = $id;
						$this->ean_matnr[$id] = $row->MATNR;
					} else {
						$this->external_id_matnr[$id] = $row->MATNR;
					}

					if ($this->feed_attributes['crosscheck'] == 't' && in_array($this->feed_attributes['identify_by'], $identify_by_array)) {
						$ean = mb_strtolower(trim($row->EAN11), 'UTF-8');
						$external_id = mb_strtolower(trim($row->IDNLF), 'UTF-8');
						$matnr_external_ids[$row->MATNR] = $external_id;
		...
		foreach ($wsResponse[0] as $wsResponseKey => $wsValue) {
			...
			if ('E_EAN' === $wsResponseKey && $wsValue->item !== null) {
				...
				foreach ($wsValue->item as $index => $row) {
					...
					if ($this->feed_attributes['identify_by'] == self::IDENTIFY_BY_EAN) {
						...
						$this->matnr_eans[$row->MATNR][] = $ean;
						$this->ean_matnr[$ean] = $row->MATNR;
					}

					if ($this->feed_attributes['crosscheck'] == 't' && in_array($this->feed_attributes['identify_by'], $identify_by_array)) {
						$external_id = $matnr_external_ids[$row->MATNR];
